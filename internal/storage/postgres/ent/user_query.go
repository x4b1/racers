...
func (uq *UserQuery) QueryRaces() *RaceQuery {
	query := &RaceQuery{config: uq.config}
...
//  WithRaces tells the query-builder to eager-loads the nodes that are connected to
// the "races" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithRaces(opts ...func(*RaceQuery)) *UserQuery {
	query := &RaceQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withRaces; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[string]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Races = []*Race{}
		}
		var (
			edgeids []string
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.RacesPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "races": %v`, err)
		}
		query.Where(race.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "races" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Races = append(nodes[i].Edges.Races, n)
			}
		}
	}
